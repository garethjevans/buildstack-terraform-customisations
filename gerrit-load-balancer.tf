...
resource "google_dns_managed_zone" "gerrit_env_dns_zone" {
...
resource "google_dns_record_set" "gerrit-dns" {
  name       = "gerrit.${google_dns_managed_zone.gerrit_env_dns_zone.dns_name}"
  depends_on = ["google_compute_global_address.gerrit-address"]
  type       = "A"
  ttl        = 300
  managed_zone = "${google_dns_managed_zone.gerrit_env_dns_zone.name}"
